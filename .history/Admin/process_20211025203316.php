...
if (isset($_GET['action']) && $_GET['action'] == 'delete-category') {
    if ($_REQUEST['id'] and $_REQUEST['id'] != '') {
        ...
        $find_prd = "SELECT * FROM `HangHoa` WHERE `HangHoa`.`MLSP` = '$category_id';";
        ...
        if (mysqli_num_rows($rs_find) > 0) {
            ...
            $sql = "DELETE FROM `HangHoa` WHERE `HangHoa`.`MLSP` = '$category_id';";
            $rs = mysqli_query($conn, $sql);
...
if (isset($_GET['action']) && $_GET['action'] == 'update-prd') {
    if (isset($_POST['submit'])) {
        ...
        $sql1 = "UPDATE `HangHoa` SET `TenHH` = '$product_name', `QuyCach` = '$product_description',`Gia` = '$product_price', `SoLuongHang` = '$product_quantity', `MLSP` = '$product_category'  WHERE `HangHoa`.`MSHH` = '$product_id'; ";
        $rs1 = mysqli_query($conn, $sql1);
