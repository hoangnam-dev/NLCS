...
<body>
                    ...
                    <form id="form-order-detail" class="product_insert-form" method="POST" enctype="multipart/form-data" role="form">
                        ...
                        <?php if ($status == "0") { ?>
                            <div class="order_detail-control">
                                <button id="confirm-order" value="<?php echo $order_id; ?>" class="button form-success_btn order-btn" onclick="return validateOrderDelivery();">Xác nhận đơn hàng</button>
                                <button id="del-order" value="<?php echo $order_id; ?>" class="button button-dangerous order_btn-del">Hủy đơn hàng</button>
                            </div>
                            <?php } else {
                            if ($status == "1") { ?>
                                <button id="complete-order" value="<?php echo $order_id; ?>" class="button form-success_btn order-btn">Hoàn thành đơn hàng</button>
                        <?php }
                        } ?>
                    </form>
                </div>
    ...
    <script>
        ...
        // Complete Order
        $("#form-order-detail").click(function(event) {
            var id = $(this).val();
            var notify = "Bạn có chắc xác nhận ĐH";
            if(confirm(notify)){
                event.preventDefault();
