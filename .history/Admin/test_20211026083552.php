...
if (isset($_GET['ins-prd'])) {
    var_dump($_POST); echo json_encode(array(
        'status' => 0,
        'message' => 'Thêm ảnh sản phẩm không thành công'
    ));
    exit;
    if (isset($_POST['product_name']) && isset($_POST['product_price']) && isset($_FILES['files']) && isset($_POST['product_quantity'])) {
    // if ( isset($_POST['submit']) && isset($_POST['product_name']) && isset($_POST['product_price']) && isset($_FILES['image_name']) && isset($_POST['product_quantity'])) {
        // echo "oke";exit;
