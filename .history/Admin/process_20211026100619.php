...
if (isset($_GET['upd-brand'])) {
    if (isset($_POST['brand_id']) && isset($_POST['brand_name']) && $_POST['brand_name']!="") {
        ...
        $sql = "UPDATE `LoaiSanPham` SET `TenLSP` = '$brand_name' WHERE `LoaiSanPham`.`MLSP` = '$brand_id';";
        $rs = mysqli_query($conn, $sql);
        if ($rs) {
            echo json_encode(array(
                'status' => 1,
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_FILES['image_name']) && !empty($_FILES['image_name']['name'][0])) {
            ...
            if (!$result) {
                ...
                $sql1 = "INSERT INTO `SanPham` (`MSSP`, `TenSP`, `MoTa`, `Gia`, `SoLuongHang`, `MLSP`) VALUES ($product_id, '$product_name', '$product_description', '$product_price', '$product_quantity', '$product_category'); ";
