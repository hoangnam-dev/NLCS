...
if (isset($_GET['ins-prd'])) {
    if (isset($_POST['product_name']) && isset($_POST['product_price']) && isset($_POST['product_quantity']) && isset($_POST['product_category']) && isset($_POST['product_brand'])) {
        ...
        $product_sale = str_replace(array('.', ','), '', $_POST['product_price']);
        ...
        if (isset($_FILES['image_name']) && !empty($_FILES['image_name']['name'][0])) {
            ...
            if (!$result) {
                ...
                $sql1 = "INSERT INTO `SanPham` (`MSSP`, `TenSP`, `MoTa`, `Gia`, `GiaBan`,  `Avatar`,`SoLuongHang`, `NgayRaMat`, `XuatXu`, `NgayTao`, `TrangThai`, `MLSP`,`MSTH`) VALUES ($product_id, '$product_name', '$product_description', '$product_price', '$product_quantity', '$product_category'); ";
                $rs1 = mysqli_query($conn, $sql1);


                if ($rs1) {
                    $last_id = mysqli_insert_id($conn);
                    $sql_img = "INSERT INTO `HinhSanPham` (`MaHinh`, `TenHinh`, `MSSP`) VALUES (NULL, '$product_img', '$product_id'); ";
