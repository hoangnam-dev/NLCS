...
// HOt Product
if (isset($_GET['hot-prd'])) {
    // echo "oke";
    // exit;
    $product_id = $_POST['id'];
    $product_hot = $_POST['hot'];
    // echo json_encode(array(
    //     'status' => 0,
    //     'message' => $product_hot
    // ));
...
if (isset($_GET['ins-customer'])) {
    if (isset($_POST['customer_name'])&&isset($_POST['customer_phone'])&&isset($_POST['customer_address'])&&isset($_POST['customer_password'])) {
        ...
        $customer_sex = isset($_POST['customer_sex']) ? $_POST['customer_sex'] : 'NULL';
...
if (isset($_GET['upd-customer'])) {
    if (isset($_POST['customer_id'])&&isset($_POST['customer_name'])&&isset($_POST['customer_phone'])&&isset($_POST['customer_address'])&&isset($_POST['customer_password'])) {
        ...
        $customer_sex = (isset($_POST['customer_sex'])) ? $_POST['customer_sex'] : '';
        $customer_password = $_POST['customer_password'];
        // echo json_encode(array (
        //     'status' => 0,
        //     'message' => $customer_email."-".$customer_sex
        // ));
        // exit;

        $sql = "UPDATE KhachHang SET `HoTenKH` = '$customer_name', `DiaChi` = '$customer_address', `SoDienThoai` = '$customer_phone', `Email` = '$customer_email', `GioiTinh` = '$customer_sex', `MatKhau` = '$customer_password' WHERE KhachHang.`MSKH` = '$customer_id';";
        $rs = mysqli_query($conn, $sql);
...
// Delete Customer
if (isset($_GET['del-customer'])) {
    if (isset($_POST['id']) && $_POST['id'] != '') {
        $customer_id = $_POST['id'];
        $find_ord = "SELECT * FROM DatHang WHERE DatHang.MSKH = '$customer_id' ";
        $rs_find = mysqli_query($conn, $find_ord);
        if(mysqli_num_rows($rs_find)>0){        
            $flag = false;
            // Cập nhật SL hàng của các đơn hàng chưa xử lý
            while($data = mysqli_fetch_assoc($rs_find)) {
                $order_id = $data['SoDonDH'];
                if($data['TrangThaiDH']!='completed'){
                // Tìm CTDH cần update SL
                    $sql_detail = "SELECT * FROM ChiTietDatHang WHERE ChiTietDatHang.SoDonDH = '$order_id'";
                    $rs_detail = mysqli_query($conn, $sql_detail);

                    // Duyệt qua từng SP trong ĐH
                    while ($detai_value = mysqli_fetch_assoc($rs_detail)) {
                        $prd_id =  $detai_value['MSSP'];
                        $order_qty =  $detai_value['SoLuong'];

                        // Tìm SP cần update SL
                        ...
                        $old_qty = $prd_values['SoLuongHang'];

                        // Update SL-hàng = SL-tồn + SL-đã-mua
                        $sql_update_qty = "UPDATE SanPham  SET SanPham.SoLuongHang = '$old_qty' + '$order_qty' WHERE SanPham.MSSP = '$prd_id';";
                        $rs_update_qty = mysqli_query($conn, $sql_update_qty);
                        if($rs_update_qty){
                            $sql_del_ord = "DELETE FROM DatHang WHERE `dathang`.`SoDonDH` = '$order_id';";
                            $rs_del_ord = mysqli_query($conn, $sql_del_ord);
                            $flag = true;
                        }
                    }
                    // echo json_encode(array (
            ...
            if($flag == true){
                ...
                echo json_encode(array (
                    'status' => 0,
                    'message' => 'Xóa ĐH của Khách hàng KHÔNG thành công' 
                ));
                exit;
            }
        }
...
if (isset($_GET['ins-staff'])) {
    if (isset($_POST['staff_name'])&&isset($_POST['staff_jobtitle'])&&isset($_POST['staff_address'])&&isset($_POST['staff_phone'])&&isset($_POST['staff_password'])) {
        ...
        // echo json_encode(array (
        //     'status' => 0,
        //     'message' => $staff_jobtitle 
        // ));
        // exit;

        $sql = "INSERT INTO `NhanVien` (`MSNV`, `HoTenNV`, `ChucVu`, `Email`, `GioiTinh`, `DiaChi`, `SoDienThoai`, `MatKhau`) VALUES (NULL, '$staff_name', '$staff_jobtitle', '$staff_email', '$staff_sex', '$staff_address', '$staff_phone','$staff_password');";
        $rs = mysqli_query($conn, $sql);
        if ($rs) {
            echo json_encode(array (
                'status' => 1,
...
if (isset($_GET['upd-staff'])) {
    if (isset($_POST['staff_name'])&&isset($_POST['staff_jobtitle'])&&isset($_POST['staff_address'])&&isset($_POST['staff_phone'])&&isset($_POST['staff_password'])) {
        ...
        $sql = "UPDATE NhanVien SET `HoTenNV` = '$staff_name', `Chucvu` = '$staff_jobtitle', `Email` = '$staff_email', `GioiTinh` = '$staff_sex', `DiaChi` = '$staff_address',`SoDienThoai` = '$staff_phone', `MatKhau` = '$staff_password' WHERE NhanVien.`MSNV` = '$staff_id';";
        $rs = mysqli_query($conn, $sql);
        if ($rs) {
            echo json_encode(array (
                'status' => 1,
...
/*=================================*/
// Order
// Confirm Order => Giao hàng
if(isset($_GET['order-comfirm'])){
    if (isset($_POST['order_id'])&&isset($_POST['order_date'])&&isset($_POST['order_delivery'])&&isset($_SESSION['admin'])) {
        ...
        // echo json_encode(array (
        //     'status' => 0,
        //     'message' => gettype(strtotime($order_delivery))
        // ));
        // exit;
        if (strtotime($order_delivery) < strtotime($order_date)) {
            echo json_encode(array (
                'status' => 0,
            ...
            if ($flag == 1) {
                ...
                if ($rs) {
                    echo json_encode(array (
                        'status' => 1,
                        'message' => 'Xác nhận đơn hàng thành công. Chuẩn bị giao hàng' 
                    ));
                    exit;
                } else {
                    echo json_encode(array (
...
// Completed Order => Hoàn thành giao hàng
if (isset($_GET['order-complete']) && isset($_POST['id']) && isset($_SESSION['admin'])) {
    $order_id = $_POST['id'];
    $admin_id = $_SESSION['admin']['MSNV'];
    ...
    if ($rs) {
        echo json_encode(array (
            'status' => 1,
            'message' => 'Giao hàng thành công. Đã xác nhận thanh toán' 
        ));
        exit;
    } else {
        echo json_encode(array (
...
// Delete Order
if (isset($_GET['del-order']) && isset($_POST['id']) && isset($_SESSION['admin'])) {
    $order_id = $_POST['id'];
    $admin_id = $_SESSION['admin']['MSNV'];
    ...
    while ($detai_value = mysqli_fetch_assoc($rs_detail)) {
        ...
        // Update SL-hàng = SL-tồn + SL-đã-mua
        $sql_update_qty = "UPDATE SanPham  SET SanPham.SoLuongHang = '$old_qty' + '$order_qty' 
    ...
    if ($rs_update_qty) {
        ...
        if ($rs_sql) {
            ...
            echo json_encode(array (
                'status' => 0,
                'message' => 'Có lỗi xảy ra. Xóa chi tiết đơn hàng không thành công' 
            ));
            exit;
        }
    } else {
        echo json_encode(array (
            'status' => 0,
            'message' => 'Có lỗi xảy ra. Cập nhật Sô lượng hàng không thành công' 
        ));
        exit;
    }
}
