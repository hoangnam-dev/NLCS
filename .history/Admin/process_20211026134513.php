...
if (isset($_GET['ins-prd'])) {
    if (isset($_POST['submit'])) {
        if (isset($_POST['product_name']) && isset($_POST['product_price']) && isset($_FILES['image_name']) && !empty($_FILES['image_name']['name']) && isset($_POST['product_quantity'])) {
            // echo "oke";exit;
            ...
            if (!$result) {
                echo "<script>alert(Thêm ảnh không thành công);location.reload();</script>";
                exit;
            } else {
                ...
                if ($rs1) {
                    ...
                    // if ($rs_img) {
                    echo "<script>alert(Thêm sản phẩm thành công);location.reload();</script>";
                    // header("location: product_manage.php");
                    exit;
                } else {
                    echo "<script>alert(Thêm sản phẩm không thành công);location.reload();</script>";
                    // header("location: product_isert.php");
                    exit;
                }
            }
        } else {
            echo "<script>alert(Thông tin sản phẩm không được rỗng);location.reload();</script>";
            // header("location: product_isert.php");
            exit;
        }
    }
