    ...
    if ($_REQUEST['id'] and $_REQUEST['id'] != '') {
        ...
        if (mysqli_num_rows($rs_find) > 0) {
            while ($product = mysqli_fetch_array($rs_find)) {
                ...
                $sql2 = "DELETE FROM `HinhSanPham` WHERE `HinhSanPham`.`MSHH` = '$product_id';";
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_FILES['image_name']) && !empty($_FILES['image_name']['name'][0])) {
            ...
            if (!$result) {
                ...
                $sql1 = "INSERT INTO `SanPham` (`MSHH`, `TenSP`, `MoTa`, `Gia`, `SoLuongHang`, `MLSP`) VALUES ($product_id, '$product_name', '$product_description', '$product_price', '$product_quantity', '$product_category'); ";
                $rs1 = mysqli_query($conn, $sql1);


                if ($rs1) {
                    $sql_img = "INSERT INTO `HinhSanPham` (`MaHinh`, `TenHinh`, `MSHH`) VALUES (NULL, '$product_img', '$product_id'); ";
...
if (isset($_GET['action']) && $_GET['action'] == 'update-prd') {
    if (isset($_POST['submit'])) {
        ...
        $sql1 = "UPDATE `SanPham` SET `TenSP` = '$product_name', `MoTa` = '$product_description',`Gia` = '$product_price', `SoLuongHang` = '$product_quantity', `MLSP` = '$product_category'  WHERE `SanPham`.`MSHH` = '$product_id'; ";
        $rs1 = mysqli_query($conn, $sql1);
        ...
        if (isset($_FILES['image_newname']) && !empty($_FILES['image_newname']['name'][0])) {
            ...
            $sql_img = "INSERT INTO `HinhSanPham` (`MaHinh`, `TenHinh`, `MSHH`) VALUES (NULL, '$product_newimg', '$product_id'); ";
            $rs_img = mysqli_query($conn, $sql_img);
...
if (isset($_GET['action']) && $_GET['action'] == 'delete-prd') {
    ...
    if ($_REQUEST['id'] and $_REQUEST['id'] != '') {
        ...
        $sql_img = "DELETE FROM `HinhSanPham` WHERE `HinhSanPham`.`MSHH` = '$product_id';";
        $rs_img = mysqli_query($conn, $sql_img);
        if ($rs_img) {
            $sql = "DELETE FROM `SanPham` WHERE `SanPham`.`MSHH` = '$product_id';";
...
if (isset($_GET['order']) && isset($_GET['id']) && isset($_SESSION['admin']) && $_GET['order'] == "delete") {
    ...
    while ($detai_value = mysqli_fetch_assoc($rs_detail)) {
        ...
        $sql_qty = "SELECT * FROM SanPham WHERE SanPham.MSHH = '$prd_id'";
