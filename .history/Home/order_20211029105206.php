            ...
            <div class="grid">
                <div class="grid_row main_content order_content">
                    <div class="grid_col-12 ctn-mb">
                        ...
                        <div class="tab">
                            <button class="tablinks  active" data-electronic="processed">Đã xử lý</button>
                            <button class="tablinks" data-electronic="waiting">Chờ xử lý</button>
                            <button class="tablinks" data-electronic="completed">Hoàn thành</button>
                        </div>

                        <!-- Đơn hàng đã xử lý -->
                        <div id="processed" class="tabcontent  active">
                            ...
                            if (mysqli_num_rows($rs1) > 0) {
                                while ($order = mysqli_fetch_array($rs1)) {
                                    ...
                                    // Định dạng ngày
                                    $ordday = date_format(date_create($order['NgayDH']),'d-m-Y');
                                    // Định dạng ngày
                                    $delivery = date_format(date_create($order['NgayGH']),'d-m-Y');
                            ?>
                                    <div class="order-content">
                                        <div class="order_content-title processed-title">
                                            <span class="order-content_title">Ngày đặt hàng: <span class="order-date"><?php echo $ordday;?></span></span>
                                        </div>
                                        <table class="order_body">
                                            ...
                                            <tr>
                                                <td colspan="2" class="order-td">
                                                    <div class="order-delivery">
                                                        Ngày giao hàng:
                                                        <span id="delivery" class="order-item_text item-delivery_time"><?php echo $delivery;?></span>
                                                    </div>
                                                </td>
                                                <td class="order-td">
                        ...
                        <div id="waiting" class="tabcontent">
                            ...
                            if (mysqli_num_rows($rs) > 0) {
                                while ($order = mysqli_fetch_array($rs)) {
                                    ...
                                    // $ordday = date_create_from_format('d-M-Y', $order['NgayDH']);
                                    // Định dạng ngày
                                    $ordday = date_format(date_create($order['NgayDH']),'d-m-Y');
                                    // echo "<pre>";
                                    // print_r($ordday);
